...
DEFAULT_DB_PATH = os.path.join(BASE_DIR, "storage", "harmonic_scanner.db")
...
class SQLiteForwardStore(BaseForwardStore):
    """
    SQLite local implementation of BaseForwardStore.
    Ideal for offline development, unit tests, and lightweight local operation.
    """
    ...
    def is_persistent(self) -> bool:
        return False # Ephemeral on cloud containers
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(sql, (
                    ...
                    record.get('p_tp1'),
                    record.get('p_tp2'),
                    record.get('confidence', 'NO_EDGE'),
                    record.get('model_name', 'None'),
                    record.get('model_version', 'NO_EDGE_NOT_DEPLOYED'),
                    record.get('geometry_quality', 80),
                    record.get('data_provider'),
                    record.get('data_mode'),
                    record.get('detector_version', '2.2.0'),
                    record.get('app_version', 'v2.2'),
    ...
    def get_prediction(self, pattern_id: str) -> Optional[Dict[str, Any]]:
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM forward_predictions WHERE pattern_id = ?", (pattern_id,))
                row = cursor.fetchone()
    ...
    def prediction_exists(self, pattern_id: str) -> bool:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT 1 FROM forward_predictions WHERE pattern_id = ? LIMIT 1", (pattern_id,))
